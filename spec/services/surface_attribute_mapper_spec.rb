...
RSpec.describe SurfaceAttributeMapper do
  describe "#call" do
    ...
    context "when volume flow is missing" do
      it "fails gracefully" do
        validation_run = create(:validation_run)
        tuple = NamelistExtractor::NamelistTuple.new(name: "Doesn't Matter", arguments: "ID='SUCK', COLOR='RED'")

        surface = described_class.new(tuple, validation_run).call

        aggregate_failures do
          expect(surface).to be_an(Surface)
          expect(surface).to have_attributes(char_id: "SUCK", volume_flow: nil)
        end
      end
    end
  end
